...
- Cache dos refinamentos no diretório resolvido (padrão ou `outputDir`): `raw.json`, `confluence.md` (se houver link citado), `business.md`, `technical.md`, `final-<uuid>.md` (com checklist de implementação ao final — histórico preservado, cada rodada gera um novo `final-<uuid>.md`), `confluence-page.json` (se já publicou no Confluence pelo menos uma vez).
- **Somente leitura na task/épico do Jira** — a skill nunca chama tool de escrita do Atlassian Rovo sobre a issue (`editJiraIssue`, `addCommentToJiraIssue`, `createIssueLink`, `transitionJiraIssue`, etc.). O refinamento é um artefato pro harness/agente (ponto de partida pra desenvolver ou delegar a task), não um artefato pro Jira.
- **Publicação no Confluence é opt-in** (etapa 10, única escrita externa da skill): campo `confluence` na entrada de `links.json` (`{ "enabled": true, "spaceId": "...", "parentId": "..." }`) — se ausente, a skill pergunta uma vez e grava a decisão (`enabled: true` ou `false`); nunca pergunta de novo depois disso. Publicação usa `createConfluencePage`/`updateConfluencePage` com `contentFormat: "markdown"` direto do `final-<uuid>.md` (cria na primeira vez, atualiza a mesma página — `pageId` guardado em `confluence-page.json` — nas rodadas seguintes).
